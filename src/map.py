...
def printMap(V):
    ...
    for arch in stealtharchers:
        a= 2*arch.position[0]
        b = 2*arch.position[1]
        if(time.time()-arch.time > 10):
            map_matrix[a][b] = sarchColor(arch) + Fore.BLACK + 'ST'
            map_matrix[a+1][b] = sarchColor(arch) + Fore.BLACK + 'CH'
            map_matrix[a][b+1] = sarchColor(arch) + Fore.BLACK + 'AR'
            map_matrix[a+1][b+1] = sarchColor(arch) + Fore.BLACK + 'ER'
