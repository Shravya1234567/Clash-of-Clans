    ...
    def break_wall(self, x, y, V,King):
        ...
        if(target.level>=3):
            ...
            if target.health <= 0:
                ...
                for t in troops:
                    r = abs(t.position[0]-x)
                    c = abs(t.position[1]-y)
                    if(r<=target.eradius and c<=target.eradius):
                        t.deal_damage(target.edamage)
                r = abs(t.position[0]-King.position[0])
                c = abs(t.position[1]-King.position[1])
                if(r<=target.eradius and c<=target.eradius):
                        King.deal_damage(target.edamage)
                target.destroy()
        else:
    ...
    def move(self, troop, King):
        ...
        if(r**2 + c**2 <= self.range**2):
            if(King==troop):
                ...
                isking = 0
            self.heal_troop(troop,King,isking)
            return
        elif(r == 0):
            ...
            if(troop.pos[0] > self.position[0]):
                    self.position[0] += 1
            else:
                    self.position[0] -= 1
    
    def heal_troop(self,troop,King,isking):
        if(self.alive == False):
            return
        if(isking == 1):
        ...
        i = troop.position[0] - 1
        j = troop.position[1] - 1
        ...
        for row in range(i, i+3):
            for col in range(j, j+3):
                ...
                for troop1 in troops:
                    if(troop1.position[0] == row and troop1.position[1] == col):
                        troop1.health += self.heal_strength
                        if troop1.health > troop1.max_health:
                            troop1.health = troop1.max_health
                if(isking==0):
...
def move_healers(King):
    for hal in healers:
        if(hal.alive == False):
            continue
        closest_troop = search_for_closest_troop(hal.position, King)
        if(closest_troop == None):
            continue
        hal.move(closest_troop,King)
...
def search_for_closest_troop(pos,King):
    ...
    for i in troops:
        dist = abs(i.position[0]-pos[0])+abs(i.position[1]-pos[1])
        if(dist < closest_dist):
            flag = 1
            closest_dist = dist
            closest_troop = i
