    ...
    def attack_target(self, target,attack):
        ...
        if target.health <= 0:
            if target.type == "wall":
                ...
                for t in troops:
                    r = abs(t.position[0]-target.position[0])
                    c = abs(t.position[1]-target.position[1])
                    if(r<=target.eradius and c<=target.eradius):
                        t.deal_damage(target.edamage)
                self.deal_damage(target.edamage)
                target.destroy()
            else:
                target.health = 0
                target.destroy()
